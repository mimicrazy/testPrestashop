...
class AttachmentCore extends ObjectModel
{
    ...
    $include = true)
    {
		...
		WHERE a.id_attachment ' .
         ($include ? 'IN' : 'NOT IN') . ' (SELECT pa.id_attachment FROM ' .
         _DB_PREFIX_ . 'product_attachment pa WHERE id_product = ' .
         (int) ($id_product) . ')');
    }
    ...
    public static function getProductAttached ($id_lang, $list)
    {
        ...
        if (is_array($list)) {
            ...
            'SELECT * FROM `' . _DB_PREFIX_ .
             'product_attachment` pa
												LEFT JOIN `' .
             _DB_PREFIX_ .
             'product_lang` pl ON (pa.`id_product` = pl.`id_product`)
