...
class AttributeCore extends ObjectModel
{
    ...
    public static function getAttributes ($id_lang, $notNull = false)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'attribute` a ON a.`id_attribute_group` = ag.`id_attribute_group`
		LEFT JOIN `' .
         _DB_PREFIX_ .
    ...
    public function isColorAttribute ()
    {
        ...
        '
			SELECT `is_color_group` FROM `' .
         _DB_PREFIX_ .
         'attribute_group` WHERE `id_attribute_group` = (
				SELECT `id_attribute_group` FROM `' .
         _DB_PREFIX_ . 'attribute` WHERE `id_attribute` = ' . (int) ($this->id) . ')
