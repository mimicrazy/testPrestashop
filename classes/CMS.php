...
class CMSCore extends ObjectModel
{
    ...
    public static function getLinks ($id_lang, $selection = NULL, $active = true)
    {
		...
		FROM ' . _DB_PREFIX_ .
         'cms c
		LEFT JOIN ' .
         _DB_PREFIX_ . 'cms_lang cl ON (c.id_cms = cl.id_cms AND cl.id_lang = ' .
         (int) ($id_lang) .
         ')
		WHERE 1
		' .
    ...
    public static function isInBlock ($id_cms, $id_block)
    {
		...
		WHERE id_block = ' .
         (int) ($id_block) . ' AND id_cms = ' . (int) ($id_cms));
    ...
    public function updatePosition ($way, $position)
    {
        ...
        '
			...
			WHERE cp.`id_cms_category` = ' .
         (int) $this->id_cms_category . ' 
        ...
        '
         ...
         ' AND `position` >= ' . (int) ($position)) . '
			AND `id_cms_category`=' .
         (int) ($movedCms['id_cms_category'])) and Db::getInstance()->Execute(
        '
			UPDATE `' . _DB_PREFIX_ . 'cms`
			SET `position` = ' . (int) ($position) . '
			WHERE `id_cms` = ' . (int) ($movedCms['id_cms']) . '
			AND `id_cms_category`=' .
         (int) ($movedCms['id_cms_category'])));
    }
