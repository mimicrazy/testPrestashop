...
class CMSCategoryCore extends ObjectModel
{
    ...
    $active = 1, $links = 0)
    {
		...
		FROM `' . _DB_PREFIX_ .
         'cms_category` c
		JOIN `' .
         _DB_PREFIX_ . 'cms_category_lang` cl ON c.`id_cms_category` = cl.`id_cms_category`
		...
		AND cl.`id_lang` = ' .
         (int) ($id_lang) . ($active ? ' AND c.`active` = 1' : '') . '
		ORDER BY c.`position`');
        if ($links == 1) {
            $link = new Link();
    ...
    public static function searchByName ($id_lang, $query, $unrestricted = false)
    {
        if ($unrestricted === true)
			...
			LEFT JOIN `' .
             ...
             (int) ($id_lang) . ')
			WHERE `name` LIKE \'%' .
             pSQL($query) . '%\' AND c.`id_cms_category` != 1');
    }
    /**
    ...
    $CMSCategory_name, $id_parent_CMSCategory)
    {
	    ...
	    LEFT JOIN `' .
         ...
         (int) ($id_lang) . ') 
	    WHERE `name`  LIKE \'' .
         pSQL($CMSCategory_name) . '\' 
		AND c.`id_cms_category` != 1 
		AND c.`id_parent` = ' .
         (int) ($id_parent_CMSCategory));
    }
    /**
    ...
    public function updatePosition ($way, $position)
    {
        ...
        '
         ...
         (int) ($movedCategory['id_parent']) . '
			AND `id_cms_category`=' .
         (int) ($movedCategory['id_cms_category'])));
    }
