...
class CacheFSCore extends Cache
{
    ...
    {
        $this->_depth = Db::getInstance()->getValue(
        'SELECT value FROM ' . _DB_PREFIX_ .
         'configuration WHERE name=\'PS_CACHEFS_DIRECTORY_DEPTH\'', false);
    ...
    {
        @file_put_contents(_PS_CACHEFS_DIRECTORY_ . 'keysCached', 
        serialize($this->_keysCached));
        @file_put_contents(_PS_CACHEFS_DIRECTORY_ . 'tablesCached', 
        serialize($this->_tablesCached));
    }
    public static function deleteCacheDirectory ()
    {
        Tools::deleteDirectory(_PS_CACHEFS_DIRECTORY_, false);
    }
    public static function createCacheDirectories ($level_depth, 
    $directory = false)
    {
        if (! $directory)
