...
class CarrierCore extends ObjectModel
{
    ...
    public function getDeliveryPriceByWeight ($totalWeight, $id_zone)
    {
        ...
        if (! isset(self::$priceByWeight[$cache_key])) {
			...
			FROM `' . _DB_PREFIX_ .
             'delivery` d
			LEFT JOIN `' .
             _DB_PREFIX_ . 'range_weight` w ON (d.`id_range_weight` = w.`id_range_weight`)
    ...
    $id_currency = NULL)
    {
        ...
        if (! isset(self::$priceByPrice[$cache_key])) {
			...
			FROM `' . _DB_PREFIX_ .
             'delivery` d
			LEFT JOIN `' .
             _DB_PREFIX_ . 'range_price` r ON d.`id_range_price` = r.`id_range_price`
			WHERE d.`id_zone` = ' . (int) ($id_zone) . '
    ...
    public static function getDeliveryPriceByRanges ($rangeTable, $id_carrier)
    {
        ...
        '
		SELECT d.`id_' .
         $rangeTable . '`, d.`id_carrier`, d.`id_zone`, d.`price`
		FROM `' . _DB_PREFIX_ .
         'delivery` d
		LEFT JOIN `' .
         _DB_PREFIX_ . $rangeTable . '` r ON r.`id_' . $rangeTable . '` = d.`id_' .
         $rangeTable .
         '`
		WHERE (d.`id_' .
         $rangeTable . '` IS NOT NULL AND d.`id_' . $rangeTable .
    ...
    $id_zone = false, $ids_group = NULL, $modules_filters = self::PS_CARRIERS_ONLY)
    {
        ...
        $sql = '
         ...
         ')
			LEFT JOIN `' .
         _DB_PREFIX_ . 'carrier_zone` cz  ON (cz.`id_carrier` = c.`id_carrier`)' .
         ($id_zone ? 'LEFT JOIN `' . _DB_PREFIX_ . 'zone` z  ON (z.`id_zone` = ' .
         (int) ($id_zone) . ')' : '') . '
			WHERE c.`deleted` ' . ($delete ? '= 1' : ' = 0') .
         ($active ? ' AND c.`active` = 1' : '') . ($id_zone ? ' AND cz.`id_zone` = ' .
         (int) ($id_zone) . '
			AND z.`active` = 1 ' : ' ');
    ...
    $activeCountries = false, $activeCarriers = false, $containStates = NULL)
    {
        ...
        '
         ...
         'country` c 
			LEFT JOIN `' .
         _DB_PREFIX_ .
         'country_lang` cl ON (c.`id_country` = cl.`id_country` AND cl.`id_lang` = 1) 
			INNER JOIN (`' .
         _DB_PREFIX_ . 'carrier_zone` cz INNER JOIN `' . _DB_PREFIX_ .
         'carrier` cr ON ( cr.id_carrier = cz.id_carrier AND cr.deleted = 0 ' . ($activeCarriers ? 'AND cr.active = 1) ' : ') ') .
         '
			LEFT JOIN `' .
         _DB_PREFIX_ . 'zone` zz ON cz.id_zone = zz.id_zone) ON zz.`id_zone` = c.`id_zone` 
			WHERE 1
			' .
        ...
        foreach ($result as $k => $row) {
            ...
            if ($shippingMethod != Carrier::SHIPPING_METHOD_FREE) {
                ...
                if (($shippingMethod ==
                 ...
                 ($shippingMethod == Carrier::SHIPPING_METHOD_PRICE and
    ...
    public function addZone ($id_zone)
    {
        ...
        '
			INSERT INTO `' .
         _DB_PREFIX_ . 'carrier_zone` (`id_carrier` , `id_zone`)
			VALUES (' . (int) ($this->id) .
         ', ' . (int) ($id_zone) . ')');
    }
    /**
    ...
    public function copyCarrierData ($oldId)
    {
        ...
        foreach ($res as $val) {
			...
			INSERT INTO `' .
             _DB_PREFIX_ .
             'range_price` (`id_carrier`, `delimiter1`, `delimiter2`)
			VALUES (' .
             (int) ($this->id) . ',' . (float) ($val['delimiter1']) . ',' .
			...
			WHERE id_carrier = ' . (int) ($oldId) . '
			AND id_range_price = ' .
             (int) ($val['id_range_price']));
            foreach ($res2 as $val2)
                Db::getInstance()->Execute(
                '
				INSERT INTO `' .
                 _DB_PREFIX_ .
                 'delivery` (`id_carrier`,`id_range_price`,`id_range_weight`,`id_zone`, `price`)
				VALUES (' .
                 (int) ($this->id) . ',' . (int) ($maxRangePrice) . ',NULL,' .
        ...
        foreach ($res as $val) {
            ...
            foreach ($res2 as $val2)
				...
				INSERT INTO `' .
                 ...
                 'delivery` (`id_carrier`,`id_range_price`,`id_range_weight`,`id_zone`, `price`)
				VALUES (' .
                 (int) ($this->id) . ',NULL,' . (int) ($maxRangeWeight) . ',' .
                 (int) ($val2['id_zone']) . ',' . (float) ($val2['price']) . ')');
        }
        // Copy existing zones
        $res = Db::getInstance()->ExecuteS(
