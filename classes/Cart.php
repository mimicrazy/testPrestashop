...
class CartCore extends ObjectModel
{
    ...
    public function delete ()
    {
		...
		FROM `' . _DB_PREFIX_ .
         'customized_data` cd
		INNER JOIN `' .
         _DB_PREFIX_ . 'customization` c ON (cd.`id_customization`= c.`id_customization`)
    ...
    public static function getTaxesAverageUsed ($id_cart)
    {
        ...
        foreach ($products as $product) {
            ...
            $ratioTax += $product['total_wt'] *
             Tax::getProductTaxRate((int) $product['id_product'], 
    ...
    public function getDiscounts ($lite = false, $refresh = false)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ . 'discount` d ON c.`id_discount` = d.`id_discount`
		WHERE `id_cart` = ' . (int) ($this->id));
    ...
    public function getProducts ($refresh = false, $id_product = false)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'product_attribute` pa ON (pa.`id_product_attribute` = cp.`id_product_attribute`)
		LEFT JOIN `' .
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'customization` cu ON (cp.`id_product` = cu.`id_product` AND cp.`id_product_attribute` = cu.`id_product_attribute` AND cu.`id_cart` = cp.`id_cart`)
		LEFT JOIN `' .
         _DB_PREFIX_ .
        ...
        foreach ($result as $row) {
            ...
            if ($this->_taxCalculationMethod == PS_TAX_EXC) {
                ...
                $row['price'] = Product::getPriceStatic((int) $row['id_product'], 
                false, (int) $row['id_product_attribute'], 6, NULL, false, true, 
            ...
            if (! isset($row['pai_id_image']) or $row['pai_id_image'] == 0) {
				...
				LEFT JOIN `' .
                 ...
                 (int) $this->id_lang . ')
				WHERE i.`id_product` = ' .
                 (int) $row['id_product'] . ' AND i.`cover` = 1');
                if (! $row2)
    ...
    public static function cacheSomeAttributesLists ($ipaList, $id_lang)
    {
        ...
        foreach ($ipaList as $id_product_attribute)
            if ((int) $id_product_attribute and
             ! array_key_exists($id_product_attribute . '-' . $id_lang, 
            self::$_attributesLists)) {
    ...
    $id_customization)
    {
		...
		' .
         ...
         '
		WHERE cp.`id_product` = ' .
         (int) $id_product . ' AND cp.`id_product_attribute` = ' .
         (int) $id_product_attribute . ' AND cp.`id_cart` = ' . (int) $this->id .
         ($id_customization ? ' AND c.`id_customization` = ' .
             ...
             Configuration::get('PS_CATALOG_MODE'))
                ...
                if ($result) {
                    if ($operator == 'up') {
                        ...
                        '
						SELECT ' .
                         (! empty($id_product_attribute) ? 'pa' : 'p') . '.`quantity`, p.`out_of_stock`
						FROM `' . _DB_PREFIX_ .
                         'product` p
						' .
						...
						WHERE p.`id_product` = ' . (int) ($id_product) .
                         (! empty($id_product_attribute) ? ' AND `id_product_attribute` = ' .
                         (int) $id_product_attribute : ''));
	...
	*/
    ...
    $id_customization, $id_product, $id_product_attribute, $operator = 'up')
    {
        ...
        $textFields = $cookie->getFamily('textFields_' . (int) ($id_product) .
         '_');
	...
	*/
    ...
    $id_customization = NULL)
    {
        ...
        if ((int) ($id_customization)) {
            ...
            (int) ($id_product), (int) ($id_product_attribute)))
                return false;
                 // refresh cache of self::_products
            $this->_products = $this->getProducts(true);
            return ($customizationQuantity == $productTotalQuantity and
             $this->deleteProduct((int) ($id_product), $id_product_attribute, 
            NULL));
        }
        ...
        if (Db::getInstance()->NumRows() and (int) ($result['quantity']))
            ...
            'UPDATE `' . _DB_PREFIX_ . 'cart_product` SET `quantity` = ' .
             ...
             (int) ($this->id) . ' AND `id_product` = ' . (int) ($id_product) .
             ($id_product_attribute != NULL ? ' AND `id_product_attribute` = ' .
             (int) ($id_product_attribute) : ''));
            /* Product deletion */
        if (Db::getInstance()->Execute(
    ...
    function getOrderShippingCost ($id_carrier = NULL, $useTax = true)
    {
        ...
        if (isset($this->id_address_delivery) and
         $this->id_address_delivery and
         Customer::customerHasAddress($this->id_customer, 
        ...
        if ($id_carrier && ! $this->isCarrierInRange($id_carrier, $id_zone))
            $id_carrier = '';
        if (empty($id_carrier) &&
         $this->isCarrierInRange(Configuration::get('PS_CARRIER_DEFAULT'), 
        $id_zone))
        ...
        if (empty($id_carrier)) {
            ...
            foreach ($result as $k => $row) {
                ...
                if (($carrier->getShippingMethod() ==
                 ...
                 ($carrier->getShippingMethod() == Carrier::SHIPPING_METHOD_PRICE and
                ...
                if ($row['range_behavior']) {
                    ...
                    $this->getTotalWeight(), $id_zone))) or
        ...
        if ($carrier->range_behavior) {
            ...
            $this->id_address_delivery))
                ...
                $id_zone = (int) $defaultCountry->id_zone;
            if (($carrier->getShippingMethod() == Carrier::SHIPPING_METHOD_WEIGHT and
             (! Carrier::checkDeliveryPriceByWeight($carrier->id, 
            $this->getTotalWeight(), $id_zone))) or
             ($carrier->getShippingMethod() == Carrier::SHIPPING_METHOD_PRICE and
             (! Carrier::checkDeliveryPriceByPrice($carrier->id, 
    ...
    public function getTotalWeight ()
    {
        if (! isset(self::$_totalWeight[$this->id])) {
			...
			LEFT JOIN `' .
             _DB_PREFIX_ .
             'product` p ON cp.`id_product` = p.`id_product`
			LEFT JOIN `' .
             _DB_PREFIX_ . 'product_attribute` pa ON cp.`id_product_attribute` = pa.`id_product_attribute`
    ...
    $products, $checkCartDiscount = false)
    {
        ...
        $discountObj->id) >= $discountObj->quantity_per_user or
         (Order::getDiscountsCustomer((int) ($cookie->id_customer), 
        $discountObj->id) + $this->getDiscountsCustomer($discountObj->id) >=
        ...
        if (($discountObj->id_customer or $discountObj->id_group) and
         ((($this->id_customer != $discountObj->id_customer) or
         ($this->id_customer == 0)) and
    ...
    function getSummaryDetails ()
    {
        ...
        new Currency((int) ($this->id_currency)))) {
            ...
            $total_free_ship = $free_ship -
             ($this->getOrderTotal(true, Cart::ONLY_PRODUCTS) +
    ...
    static function getNonOrderedCarts ($dateFrom, $dateTo)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ . 'cart_product` cp ON cart.`id_cart` = cp.`id_cart`
		LEFT JOIN `' .
         _DB_PREFIX_ . 'cart_discount` cd ON cart.`id_cart` = cd.`id_cart`
		LEFT JOIN `' .
         _DB_PREFIX_ . 'customer` c ON cart.`id_customer` = c.`id_customer`
		WHERE cart.`id_cart` NOT IN (SELECT `id_cart` FROM `' .
         _DB_PREFIX_ .
         'orders`)
    ...
    public function checkQuantities ()
    {
        ...
        foreach ($this->getProducts() as $product)
            if (! $product['active'] or
             (! $product['allow_oosp'] and
    ...
    public static function lastNoneOrderedCart ($id_customer)
    {
        ...
        '
			...
			LEFT JOIN ' .
         _DB_PREFIX_ . 'orders o ON (c.`id_cart` = o.`id_cart`)
			WHERE c.`id_customer` = ' .
         (int) ($id_customer) . ' AND o.`id_cart` IS NULL
	...
	*/
    ...
    {
        global $cookie;
        unset($cookie->{'textFields_' . (int) ($id_product) . '_' .
         (int) ($index)});
        return true;
    }
    /*
	...
	*/
    ...
    public function duplicate ()
    {
		...
		FROM ' . _DB_PREFIX_ .
         'customization c
		LEFT JOIN ' .
         _DB_PREFIX_ . 'customized_data cd ON cd.id_customization = c.id_customization
        ...
        foreach ($customsById as $customizationId => $val) {
			...
			INSERT INTO `' .
             _DB_PREFIX_ .
             'customization` (id_cart, id_product_attribute, id_product, quantity)
			VALUES(' .
             (int) $cart->id . ', ' . (int) $val['id_product_attribute'] . ', ' .
    ...
    public static function isGuestCartByCartId ($id_cart)
    {
        ...
        '
			...
			LEFT JOIN `' .
         _DB_PREFIX_ . 'cart` ca ON (ca.`id_customer` = cu.`id_customer`)
    ...
    public function isCarrierInRange ($id_carrier, $id_zone)
    {
        ...
        if ($shippingMethod == Carrier::SHIPPING_METHOD_FREE)
            return true;
        if ($shippingMethod == Carrier::SHIPPING_METHOD_WEIGHT and
         (Carrier::checkDeliveryPriceByWeight((int) $id_carrier, 
        $this->getTotalWeight(), $id_zone)))
            return true;
        if ($shippingMethod == Carrier::SHIPPING_METHOD_PRICE and
         (Carrier::checkDeliveryPriceByPrice((int) $id_carrier, 
        $this->getOrderTotal(true, Cart::BOTH_WITHOUT_SHIPPING), $id_zone, 
        (int) $this->id_currency)))
            return true;
        return false;
