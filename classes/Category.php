...
class CategoryCore extends ObjectModel
{
    ...
    public function delete ()
    {
		...
		FROM `' . _DB_PREFIX_ .
         'product`
		WHERE `id_product` NOT IN (SELECT `id_product` FROM `' .
    ...
    public function recalculateLevelDepth ($id_category)
    {
        ...
        foreach ($categories as $sub_category) {
            ...
            '
				...
				SET level_depth = ' .
             (int) ($level['level_depth'] + 1) . '
				WHERE id_category = ' .
             (int) $sub_category['id_category']);
            /* Recursive call */
    ...
    $sql_filter = '', $sql_sort = '', $sql_limit = '')
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'category_lang` cl ON c.`id_category` = cl.`id_category`
		WHERE 1 ' .
         $sql_filter . ' ' . ($id_lang ? 'AND `id_lang` = ' . (int) ($id_lang) : '') . '
		' . ($active ? 'AND `active` = 1' : '') . '
		' .
    ...
    public static function searchByName ($id_lang, $query, $unrestricted = false)
    {
        if ($unrestricted === true)
			...
			LEFT JOIN `' .
             ...
             (int) ($id_lang) . ')
			WHERE `name` LIKE \'%' .
             pSQL($query) . '%\' AND c.`id_category` != 1');
    }
    /**
    ...
    public function getParentsCategories ($idLang = null)
    {
        ...
        while (true) {
            ...
            '
             ...
             (int) ($idLang) . ')
				WHERE c.`id_category` = ' .
             (int) $idCurrent . ' AND c.`id_parent` != 0
			');
            $categories[] = $result[0];
    ...
    public function updatePosition ($way, $position)
    {
        ...
        '
         ...
         (int) ($position)) . '
			AND `id_parent`=' .
         (int) ($movedCategory['id_parent'])) and Db::getInstance()->Execute(
        '
			UPDATE `' . _DB_PREFIX_ . 'category`
			SET `position` = ' . (int) ($position) . '
			WHERE `id_parent` = ' .
         (int) ($movedCategory['id_parent']) . '
			AND `id_category`=' .
         (int) ($movedCategory['id_category'])));
        Module::hookExec('categoryUpdate');
        return $result;
    }
    /**
    ...
    public static function getUrlRewriteInformations ($id_category)
    {
		...
		LEFT JOIN  `' .
         _DB_PREFIX_ . 'lang` AS l ON c.`id_lang` = l.`id_lang`
		WHERE c.`id_category` = ' . (int) $id_category . '
		AND l.`active` = 1');
    }
    ...
    public function getWsNbProductsRecursive ()
    {
		...
		FROM `' . _DB_PREFIX_ . 'category`
		WHERE nleft > ' . (int) $this->nleft . ' AND nright < ' .
         (int) $this->nright . ' AND active = 1 UNION SELECT ' . (int) $this->id .
         ')');
        if (! $result)
            return - 1;
