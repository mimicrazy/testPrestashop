...
class ConfigurationCore extends ObjectModel
{
    ...
    public static function deleteByName ($key)
    {
        ...
        'DELETE FROM `' . _DB_PREFIX_ .
         ...
         _DB_PREFIX_ . 'configuration` WHERE `name` = \'' . pSQL($key) . '\')') and
        ...
        if (! is_array($values)) {
            ...
            foreach ($values as $id_lang => $value) {
                ...
                'INSERT INTO `' . _DB_PREFIX_ .
                 'configuration_lang` (`id_configuration`, `id_lang`, `value`, `date_upd`)
										VALUES (' .
                 $conf['id_configuration'] . ', ' . (int) ($id_lang) . ', \'' .
                 $value .
                 '\', NOW())
    ...
    $sql_limit)
    {
		...
		SELECT DISTINCT main.`' .
         $this->identifier . '` FROM `' . _DB_PREFIX_ . $this->table . '` main
		' . $sql_join . '
