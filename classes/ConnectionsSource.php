...
class ConnectionsSourceCore extends ObjectModel
{
        ...
        if (isset($_SERVER['HTTP_REFERER']) and
         Validate::isAbsoluteUrl($_SERVER['HTTP_REFERER'])) {
            ...
            if ((preg_replace('/^www./', '', $parsed['host']) ==
             preg_replace('/^www./', '', Tools::getHttpHost(false, false))) and
             ! strncmp($parsed['path'], $parsed_host['path'], 
    ...
    public static function getOrderSources ($id_order)
    {
		...
		INNER JOIN ' .
         _DB_PREFIX_ . 'guest g ON g.id_customer = o.id_customer
		INNER JOIN ' .
         _DB_PREFIX_ .
         'connections co  ON co.id_guest = g.id_guest
		INNER JOIN ' .
         _DB_PREFIX_ . 'connections_source cos ON cos.id_connections = co.id_connections
