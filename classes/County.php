...
class CountyCore extends ObjectModel
{
    ...
    public static function getIdCountyByZipCode ($id_state, $zip_code)
    {
        if (! isset(self::$_cache_county_zipcode[$id_state . '-' . $zip_code])) {
            ...
            '
			SELECT DISTINCT c.`id_county` FROM `' . _DB_PREFIX_ .
             'county` c
			LEFT JOIN `' .
             _DB_PREFIX_ . 'county_zip_code` cz ON (c.`id_county` = cz.`id_county`)
			WHERE `id_state` = ' . (int) $id_state . '
    ...
    public function isZipCodeRangePresent ($zip_codes)
    {
        ...
        if ($to != 0) {
			...
			LEFT JOIN `' .
             _DB_PREFIX_ . 'county` c ON (c.`id_county` = cz.`id_county`)
			LEFT JOIN `' .
             _DB_PREFIX_ . 'state` s ON (s.`id_state` = c.`id_state`)
			WHERE `from_zip_code` >= ' . (int) $from . '
			AND `to_zip_code` <= ' . (int) $to . '
			AND s.`id_country` = (SELECT `id_country`
										 FROM `' . _DB_PREFIX_ . 'state` s
										 LEFT JOIN `' .
             _DB_PREFIX_ . 'county` c ON (c.`id_state` = s.`id_state`)
										 WHERE `id_county` = ' . (int) $this->id . '
										)');
        }
    ...
    public function isZipCodePresent ($zip_code)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'state` s ON (s.`id_state` = c.`id_state`)
		WHERE
		(`from_zip_code` <= ' .
         (int) $zip_code . ' AND `to_zip_code` >= ' . (int) $zip_code . ')
		OR
