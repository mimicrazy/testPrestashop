...
class CustomerCore extends ObjectModel
{
    ...
    public function getByEmail ($email, $passwd = NULL)
    {
		...
		AND `email` = \'' . pSQL($email) .
         '\'
		' .
         (isset($passwd) ? 'AND `passwd` = \'' . md5(pSQL(_COOKIE_KEY_ .
         $passwd)) . '\'' : '') . '
		AND `deleted` = 0
		AND `is_guest` = 0');
        if (! $result)
    ...
    public function cantChangeemail ()
    {
		...
		FROM `' . _DB_PREFIX_ .
         'customer`
		WHERE `email` = \'' .
         pSQL($this->email) . '\' AND `id_customer` != ' . (int) ($this->id));
        return $result['total'];
    }
    /**
    ...
    public function getAddresses ($id_lang)
    {
		...
		WHERE `id_lang` = ' .
         (int) ($id_lang) . ' AND `id_customer` = ' . (int) ($this->id) .
         ' AND a.`deleted` = 0');
    }
    /**
    ...
    public function getConnections ($nb = 10)
    {
		...
		FROM `' . _DB_PREFIX_ .
         'connections`
		WHERE `id_guest` IN (SELECT `id_guest` FROM `' .
         _DB_PREFIX_ . 'guest` WHERE `id_customer` = ' . (int) ($this->id) . ')
	...
	*/
    ...
    public function getBoughtProducts ()
    {
		...
		WHERE o.valid = 1 AND o.`id_customer` = ' .
         (int) ($this->id));
    }
    /**
     * @deprecated
