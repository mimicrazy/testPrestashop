...
class DiscountCore extends ObjectModel
{
    ...
    $active = false, $includeGenericOnes = true, $stock = false)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'discount_type` dt ON dt.`id_discount_type` = d.`id_discount_type`
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'discount_type_lang` dtl ON (dt.`id_discount_type` = dtl.`id_discount_type` AND dtl.`id_lang` = ' .
         (int) ($id_lang) . ')
		WHERE (`id_customer` = ' . (int) ($id_customer) .
         '
		OR `id_group` IN (SELECT `id_group` FROM `' .
         _DB_PREFIX_ . 'customer_group` WHERE `id_customer` = ' .
         (int) ($id_customer) . ')' .
        ...
        switch ($this->id_discount_type) {
            ...
            case 1:
                ...
                foreach ($products as $product)
                    ...
                    $categories))
                        if ($this->cumulable_reduction or
                         (! $product['reduction_applies'] and
                         ! $product['on_sale']))
    ...
    public function updateCategories ($categories)
    {
        ...
        foreach ($categories as $category) {
			...
			FROM `' . _DB_PREFIX_ .
             'discount_category`
			WHERE `id_discount`=' .
             (int) ($this->id) . ' AND `id_category`=' . (int) ($category));
    ...
    public static function getVouchersToCartDisplay ($id_lang, $id_customer)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'discount_lang` dl ON (d.`id_discount` = dl.`id_discount`)
		WHERE d.`active` = 1
		AND d.`date_from` <= \'' .
         pSQL(date('Y-m-d H:i:s')) . '\' AND d.`date_to` >= \'' .
         pSQL(date('Y-m-d H:i:s')) . '\'
		AND dl.`id_lang` = ' . (int) ($id_lang) .
         '
		AND d.`cart_display` = 1 AND d.`quantity` > 0
		AND ((d.`id_customer` = 0 AND d.`id_group` = 0) ' .
         ($id_customer ? 'OR (d.`id_customer` = ' . $id_customer .
         '
		OR d.`id_group` IN (SELECT `id_group` FROM `' .
         _DB_PREFIX_ . 'customer_group` WHERE `id_customer` = ' .
         (int) ($id_customer) . ')))' : 'OR d.`id_group` = 1)'));
    }
