...
class FrontControllerCore
{
    ...
    public function init ()
    {
        ...
        $cookieLifetime = (time() +
         (((int) Configuration::get('PS_COOKIE_LIFETIME_FO') > 0 ? (int) Configuration::get(
        ...
        if ((int) $cookie->id_cart) {
            ...
            elseif (intval(Configuration::get('PS_GEOLOCATION_ENABLED')) and
             ! in_array(strtoupper($cookie->iso_code_country), 
        ...
        'come_from' => Tools::getHttpHost(true, true) .
    ...
    protected function geolocationManagement ()
    {
        ...
        if (! in_array($_SERVER['SERVER_NAME'], array('localhost', '127.0.0.1'))) {
            ...
            if (file_exists(_PS_GEOIP_DIR_ . 'GeoLiteCity.dat')) {
                if (! isset($cookie->iso_code_country) or
                 (isset($cookie->iso_code_country) and
                 ! in_array(strtoupper($cookie->iso_code_country), 
                explode(';', Configuration::get('PS_ALLOWED_COUNTRIES'))))) {
                    include_once (_PS_GEOIP_DIR_ . 'geoipcity.inc');
                    include_once (_PS_GEOIP_DIR_ . 'geoipregionvars.php');
                    $gi = geoip_open(realpath(_PS_GEOIP_DIR_ .
                ...
                if (isset($cookie->iso_code_country) and
                 (int) ($id_country = Country::getByIso(
    ...
    public function setMedia ()
    {
        ...
        if (Tools::isSubmit('live_edit') and Tools::getValue('ad') and
         (Tools::getValue('liveToken') ==
         sha1(Tools::getValue('ad') . _COOKIE_KEY_))) {
            Tools::addJS(
    ...
    public function displayFooter ()
    {
        ...
        //live edit
        if (Tools::isSubmit('live_edit') and $ad = Tools::getValue('ad') and
         (Tools::getValue('liveToken') ==
         sha1(Tools::getValue('ad') . _COOKIE_KEY_))) {
