...
class HookCore extends ObjectModel
{
    ...
    public static function preloadModulesFromHooks ($position = false)
    {
		...
		FROM `' . _DB_PREFIX_ . 'hook` h
		INNER JOIN `' .
         _DB_PREFIX_ . 'hook_module` hm ON (h.id_hook = hm.id_hook)
		INNER JOIN `' .
         _DB_PREFIX_ . 'module` as m    ON (m.id_module = hm.id_module)
		' .
         ($position ? 'WHERE h.`position` = 1' : ''));
    ...
    public static function getModuleFromHook ($id_hook, $id_module)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'hook_module` hm ON (hm.id_module = m.id_module)
		WHERE hm.id_hook = ' .
         (int) ($id_hook) . ' AND m.id_module = ' . (int) ($id_module));
    }
