...
class ImageCore extends ObjectModel
{
    ...
    public static function getImages ($id_lang, $id_product)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'image_lang` il ON (i.`id_image` = il.`id_image`)
		WHERE i.`id_product` = ' .
         (int) $id_product . ' AND il.`id_lang` = ' . (int) $id_lang . '
    ...
    public function positionImage ($position, $direction)
    {
		...
		WHERE `id_product` = ' . (int) ($this->id_product) . '
		AND `position` = ' .
         ($direction ? $position - 1 : $position + 1));
        Db::getInstance()->Execute(
        '
		UPDATE `' . _DB_PREFIX_ . 'image`
    ...
    public function createImgFolder ()
    {
        ...
        if (! file_exists(_PS_PROD_IMG_DIR_ . $this->getImgFolder())) {
            ...
            if ($success &&
             ! file_exists(_PS_PROD_IMG_DIR_ . $this->getImgFolder() .
             'index.php') && file_exists($this->source_index))
