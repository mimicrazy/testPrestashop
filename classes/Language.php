...
class LanguageCore extends ObjectModel
{
    ...
    private function _generateFiles ($newIso = null)
    {
        ...
        foreach ($this->translationsFilesAndVars as $file => $var)
            if (! file_exists(_PS_TRANSLATIONS_DIR_ . $iso_code . '/' . $file .
             '.php'))
                file_put_contents(
...
?>');
    ...
    public function moveToIso ($newIso)
    {
        ...
        foreach ($modulesList as $moduleDir) {
            ...
            _PS_MODULE_DIR_ . $moduleDir . '/mails/' . $this->iso_code))
                rename(_PS_MODULE_DIR_ . $moduleDir . '/mails/' .
                 $this->iso_code, _PS_MODULE_DIR_ . $moduleDir . '/mails/' .
                 $newIso);
            if (file_exists(
            _PS_MODULE_DIR_ . $moduleDir . '/' . $this->iso_code . '.php'))
                rename(_PS_MODULE_DIR_ . $moduleDir . '/' . $this->iso_code .
                 '.php', _PS_MODULE_DIR_ . $moduleDir . '/' . $newIso . '.php');
        }
        foreach ($this->_getThemesList() as $theme => $data) {
            ...
            _PS_ALL_THEMES_DIR_ . $theme . '/lang/' . $this->iso_code . '.php'))
                ...
                _PS_ALL_THEMES_DIR_ . $theme . '/lang/' . $this->iso_code .
                 '.php', _PS_ALL_THEMES_DIR_ . $theme . '/lang/' . $newIso .
                 '.php');
            if (file_exists(
            _PS_ALL_THEMES_DIR_ . $theme . '/mails/' . $this->iso_code))
                rename(_PS_ALL_THEMES_DIR_ . $theme . '/mails/' .
                 $this->iso_code, _PS_ALL_THEMES_DIR_ . $theme . '/mails/' .
                 $newIso);
    ...
    $theme_to = false, $select = false, $check = false, $modules = false)
    {
        ...
        if (! $check or ($check and (string) $iso_from != 'en')) {
            ...
            foreach ($module_theme_files as $module)
                if ($module !== '.' and $module != '..' and $module !== '.svn' and
                 file_exists(
    ...
    public function delete ()
    {
        ...
        foreach ($modList as $mod) {
            ...
            if (file_exists(_PS_MODULE_DIR_ . $mod . '/' . $this->iso_code .
             '.php')) {
        ...
        foreach ($tos as $to)
            foreach ($files_copy as $file) {
                ...
                if (file_exists($to . $name))
                    unlink($to . $name);
                if (file_exists(dirname(__FILE__) . '/../img/l/' . $this->id .
                 '.jpg'))
            ...
            if (@fsockopen('www.prestashop.com', 80)) {
                ...
                foreach ($tos as $to)
                    foreach ($files_copy as $file) {
                        ...
                        copy(dirname(__FILE__) . '/../img/l' . $file, $to .
                         $name);
                    }
                return true;
            } else
