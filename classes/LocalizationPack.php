...
class LocalizationPackCore
{
            ...
            foreach ($xml->taxes->taxRulesGroup as $group) {
                ...
                foreach ($group->taxRule as $rule) {
                    ...
                    if (! isset($rule_attributes['id_tax']) ||
                     ! array_key_exists(strval($rule_attributes['id_tax']), 
                    $assoc_taxes))
                        continue;
                         // Default values
                    $id_state = (int) isset(
                    ...
                    $county_behavior = 0;
                    if ($id_state) {
                        if (isset($rule_attributes['state_behavior']) &&
                         in_array($rule_attributes['state_behavior'], 
                        $available_behavior))
                        ...
                        if (isset($rule_attributes['county_behavior']) &&
                         in_array($rule_attributes['state_behavior'], 
                        $available_behavior))
    ...
    protected function _installCurrencies ($xml, $install_mode = false)
    {
        if (isset($xml->currencies->currency)) {
            ...
            'http://www.prestashop.com/xml/currencies.xml') and
