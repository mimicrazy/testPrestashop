...
class LoggerCore extends ObjectModel
{
    ...
    protected $fieldsValidate = array('id_log' => 'isUnsignedId', 
    'severity' => 'isInt', 'error_code' => 'isUnsignedInt', 
    'message' => 'isMessage', 'object_id' => 'isUnsignedInt', 
    'object_type' => 'isName');
    protected $table = 'log';
    ...
    public static function sendByMail ($log)
    {
        ...
        if (intval(Configuration::get('PS_LOGS_BY_EMAIL')) <=
         intval($log->severity))
            Mail::Send((int) Configuration::get('PS_LANG_DEFAULT'), 'log_alert', 
            Mail::l('Log: You have a new alert from your shop'), array(), 
     ...
     */
    public static function addLog ($message, $severity = 1, $errorCode = NULL, 
    $objectType = NULL, $objectId = NULL, $allowDuplicate = false)
    {
        $log = new Logger();
        ...
        if ($allowDuplicate or ! $log->_isPresent()) {
            $res = $log->add();
            if ($res) {
                self::$_is_present[$log->getHash()] = isset(
                self::$_is_present[$log->getHash()]) ? self::$_is_present[$log->getHash()] +
                 1 : 1;
                return true;
            }
        }
    ...
    public function getHash ()
    {
        if (empty($this->hash))
            $this->hash = md5(
            $this->message . $this->severity . $this->error_code .
    ...
    private function _isPresent ()
    {
        if (! isset(self::$_is_present[md5($this->message)]))
            self::$_is_present[$this->getHash()] = Db::getInstance()->getValue(
            'SELECT COUNT(*)
