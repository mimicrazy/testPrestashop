...
abstract class ModuleCore
{
    ...
    public function registerHook ($hook_name)
    {
        ...
        $result = Db::getInstance()->getRow(
        '
		SELECT hm.`id_module` FROM `' .
         _DB_PREFIX_ . 'hook_module` hm, `' . _DB_PREFIX_ . 'hook` h
		...
		VALUES (' .
         (int) ($this->id) . ', ' . (int) ($result['id_hook']) . ', ' .
    ...
    public function editExceptions ($id_hook, $excepts)
    {
        ...
        '
				...
				WHERE `id_module` = ' .
         (int) ($this->id) . ' AND `id_hook` =' . (int) ($id_hook));
    ...
    public static function getModulesOnDisk ($useConfig = false)
    {
        ...
        foreach ($modules_dir as $module) {
                ...
                if (! count($errors) and (int) $xml_module->need_instance == 0 and
                 ! $needNewConfigFile) {
                    ...
                    if (isset($xml_module->confirmUninstall))
                        ...
                        $xml_module->confirmUninstall), (string) $xml_module->name);
            ...
            if (! $useConfig or ! $xml_exist or
             (isset($xml_module->need_instance) and
             (int) $xml_module->need_instance == 1) or $needNewConfigFile) {
    ...
    public static function getModulesDirOnDisk ()
    {
        ...
        foreach ($modules as $name) {
            if (is_dir(_PS_MODULE_DIR_ . $name) &&
             Tools::file_exists_cache(_PS_MODULE_DIR_ . $name . '/' . $name .
             '.php')) {
    ...
    public static function getModulesInstalled ($position = 0)
    {
		...
		' . ($position ? '
		LEFT JOIN `' .
         _DB_PREFIX_ . 'hook_module` hm ON m.`id_module` = hm.`id_module`
    ...
    public static function hookExec ($hook_name, $hookArgs = array(), $id_module = NULL)
    {
        ...
        if (! isset(self::$_hookModulesCache)) {
			...
			FROM `' . _DB_PREFIX_ . 'module` m
			LEFT JOIN `' .
             _DB_PREFIX_ . 'hook_module` hm ON hm.`id_module` = m.`id_module`
        ...
        foreach (self::$_hookModulesCache[$hook_name] as $array) {
            ...
            if (! ($moduleInstance = Module::getInstanceByName($array['module'])))
                continue;
            $exceptions = $moduleInstance->getExceptions((int) $array['id_hook'], 
            (int) $array['id_module']);
            ...
            if (is_callable(array($moduleInstance, 'hook' . $hook_name))) {
                ...
                if ($array['live_edit'] &&
                 ((Tools::isSubmit('live_edit') and Tools::getValue('ad') and
                 ...
                 sha1(Tools::getValue('ad') . _COOKIE_KEY_))))) {
                    ...
                    $output .= '<script type="text/javascript"> modules_list.push(\'' .
                     ...
                     '\');</script>
								<div id="hook_' .
                     $array['id_hook'] . '_module_' . $moduleInstance->id .
                     '_moduleName_' . $moduleInstance->name .
                     '"
								class="dndModule" style="border: 1px dotted red;' .
                     (! strlen($display) ? 'height:50px;' : '') . '">
								<span><img src="' .
                     $moduleInstance->_path . '/logo.gif">' .
                     $moduleInstance->displayName .
                     '<span style="float:right">
							 	<a href="#" id="' .
                     $array['id_hook'] . '_' . $moduleInstance->id . '" class="moveModule">
							 		<img src="' . _PS_ADMIN_IMG_ .
                     'arrow_out.png"></a>
							 	<a href="#" id="' .
                     $array['id_hook'] . '_' . $moduleInstance->id . '" class="unregisterHook">
        ...
        return ($live_edit ? '<script type="text/javascript">hooks_list.push(\'' .
         $hook_name . '\'); </script><!--<div id="add_' . $hook_name .
         '" class="add_module_live_edit">
            ...
            foreach ($result as $module)
                ...
                $module['name'])) and
                 is_callable(array($moduleInstance, 'hookpayment')))
                    if (! $moduleInstance->currencies or
                     ($moduleInstance->currencies and
                     sizeof(
    ...
    public static function getPaymentModules ()
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'module` m ON m.`id_module` = mc.`id_module`
		INNER JOIN `' .
         _DB_PREFIX_ .
         'module_group` mg ON (m.`id_module` = mg.`id_module`)
		INNER JOIN `' .
         _DB_PREFIX_ .
    ...
    public function updatePosition ($id_hook, $way, $position = NULL)
    {
		...
		AND `id_hook`=' . (int) ($from['id_hook'])) and
         Db::getInstance()->Execute(
        '
		UPDATE `' . _DB_PREFIX_ . 'hook_module`
		SET `position`=' . (int) ($to['position']) .
         '
		WHERE `' .
         pSQL($this->identifier) . '` = ' . (int) ($from[$this->identifier]) .
    ...
    public function displayError ($error)
    {
		...
		<div class="module_error alert error">
			<img src="' .
         _PS_IMG_ . 'admin/warning.gif" alt="" title="" /> ' . $error . '
		</div>';
        $this->error = true;
        return $output;
    }
    ...
    public function isRegisteredInHook ($hook)
    {
		...
		FROM `' . _DB_PREFIX_ . 'hook_module` hm
		LEFT JOIN `' .
         _DB_PREFIX_ . 'hook` h ON (h.`id_hook` = hm.`id_hook`)
...
<module>
	<name>' . $this->name .
         '</name>
	<displayName><![CDATA[' .
         Tools::htmlentitiesUTF8($this->displayName) . ']]></displayName>
	<version><![CDATA[' . $this->version .
         ']]></version>
	<description><![CDATA[' .
         Tools::htmlentitiesUTF8($this->description) . ']]></description>
	<author><![CDATA[' .
         Tools::htmlentitiesUTF8($this->author) .
         ']]></author>
	<tab><![CDATA[' .
         Tools::htmlentitiesUTF8($this->tab) . ']]></tab>' .
         (isset($this->confirmUninstall) ? "\n\t" . '<confirmUninstall>' .
         $this->confirmUninstall . '</confirmUninstall>' : '') . '
	<is_configurable>' .
         (int) method_exists($this, 'getContent') .
         '</is_configurable>
	<need_instance>' .
         (int) $this->need_instance . '</need_instance>' .
         (isset($this->limited_countries) ? "\n\t" . '<limited_countries>' .
