...
abstract class ModuleGridCore extends Module
{
    ...
    public function create ($render, $type, $width, $height, $start, $limit, 
    $sort, $dir)
    {
    ...
    public static function engine ($params)
    {
        ...
        $grider = 'grider.php?render=' . $render . '&module=' .
         Tools::safeOutput(Tools::getValue('module'));
        global $cookie;
        $grider .= '&id_employee=' . (int) ($cookie->id_employee);
        $grider .= '&id_lang=' . (int) ($cookie->id_lang);
        if (! isset($params['width']) or
         ! Validate::IsUnsignedInt($params['width']))
            $params['width'] = 600;
        if (! isset($params['height']) or
         ! Validate::IsUnsignedInt($params['height']))
            $params['height'] = 920;
        if (! isset($params['start']) or
         ! Validate::IsUnsignedInt($params['start']))
            $params['start'] = 0;
        if (! isset($params['limit']) or
         ! Validate::IsUnsignedInt($params['height']))
            $params['limit'] = 40;
        $grider .= '&width=' . $params['width'];
        $grider .= '&height=' . $params['height'];
        if (isset($params['start']) and Validate::IsUnsignedInt(
        $params['start']))
            $grider .= '&start=' . $params['start'];
        if (isset($params['limit']) and Validate::IsUnsignedInt(
        $params['limit']))
            $grider .= '&limit=' . $params['limit'];
        if (isset($params['type']) and Validate::IsName($params['type']))
            $grider .= '&type=' . $params['type'];
        if (isset($params['option']) and
         Validate::IsGenericName($params['option']))
        ...
        require_once (dirname(__FILE__) . '/../modules/' . $render . '/' .
         $render . '.php');
        return call_user_func(array($render, 'hookGridEngine'), $params, 
        $grider);
    }
