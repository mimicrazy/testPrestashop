...
class MySQLCore extends Db
{
    public function connect ()
    {
        ...
        if ($this->_link = mysql_connect($this->_server, $this->_user, 
        $this->_password)) {
    ...
    public function getValue ($query, $use_cache = 1)
    {
        ...
        if ($this->_link and $this->_result = mysql_query($query, $this->_link) and
         is_array($tmpArray = mysql_fetch_assoc($this->_result))) {
            $this->_lastCached = false;
    ...
    public function displayMySQLError ($query = false)
    {
        ...
        if ($webservice_call && mysql_errno($this->_link))
            ...
            '[SQL Error] ' . mysql_error($this->_link) . '. Query was : ' .
             $query, 97);
        elseif (_PS_DEBUG_SQL_ and mysql_errno($this->_link) and
         ! defined('PS_INSTALLATION_IN_PROGRESS')) {
            if ($query)
                die(
                Tools::displayError(
                mysql_error($this->_link) . '<br /><br /><pre>' . $query .
                 '</pre>'));
     ...
     * @return integer
     */
    public static function tryToConnect ($server, $user, $pwd, $db, 
    $newDbLink = true)
    {
