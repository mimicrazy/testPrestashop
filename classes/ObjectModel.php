...
abstract class ObjectModelCore
{
        ...
        if ($id) {
            ...
            self::$_cache[$this->table][(int) ($id)][(int) ($id_lang)]))
				...
				FROM `' . _DB_PREFIX_ . $this->table .
                 '` a ' .
                 ($id_lang ? ('LEFT JOIN `' . pSQL(_DB_PREFIX_ . $this->table) .
                 '_lang` b ON (a.`' . $this->identifier . '` = b.`' .
                 $this->identifier) . '` AND `id_lang` = ' . (int) ($id_lang) .
                 ')' : '') . ' WHERE a.`' . $this->identifier . '` = ' .
                 (int) ($id));
            ...
            if ($result) {
                ...
                if (! $id_lang and
                 method_exists($this, 'getTranslationsFieldsChild')) {
					...
					SELECT * 
					FROM `' .
                     pSQL(_DB_PREFIX_ . $this->table) . '_lang` 
    ...
    public function validateFieldsLang ($die = true, $errorReturn = false)
    {
        ...
        foreach ($this->fieldsRequiredLang as $fieldArray) {
            if (! is_array($this->{$fieldArray}))
                continue;
            if (! $this->{$fieldArray} or ! sizeof($this->{$fieldArray}) or
             ($this->{$fieldArray}[$defaultLanguage] !== '0' and
             empty($this->{$fieldArray}[$defaultLanguage]))) {
                if ($die)
                    die(
                    Tools::displayError() . ' (' . get_class($this) . '->' .
                     $fieldArray . ' ' .
            ...
            if (($value = Tools::getValue($field, $this->{$field})) == false and
             (string) $value != '0')
                if (! $this->id or $field != 'passwd')
                    ...
                    $htmlentities) . '</b> ' . Tools::displayError(
                    'is required.');
    ...
    public function getWebserviceParameters ($wsParamsAttributeName = NULL)
    {
        ...
        if (isset($this->{$wsParamsAttributeName}['retrieveData']) &&
         isset(
    ...
    $sql_limit)
    {
		...
		SELECT DISTINCT main.`' .
         $this->identifier . '` FROM `' . _DB_PREFIX_ . $this->table . '` AS main
		' . $sql_join . '
		WHERE 1 ' . $sql_filter . '
		' . ($sql_sort != '' ? $sql_sort : '') . '
		' .
    ...
    public function deleteImage ()
    {
        ...
        if ($this->image_dir) {
            if (file_exists($this->image_dir . $this->id . '.' .
             $this->image_format) &&
