...
class OrderCore extends ObjectModel
{
    ...
    public function getHistory ($id_lang, $id_order_state = false, $no_hidden = false)
    {
        ...
        if (! isset(self::$_historyCache[$this->id . '_' . $id_order_state]) or
         $no_hidden) {
			...
			LEFT JOIN `' .
             _DB_PREFIX_ .
             'order_history` oh ON o.`id_order` = oh.`id_order`
			LEFT JOIN `' .
             _DB_PREFIX_ .
			...
			LEFT JOIN `' .
             ...
             (int) ($id_lang) . ')
			LEFT JOIN `' .
             _DB_PREFIX_ . 'employee` e ON e.`id_employee` = oh.`id_employee`
    ...
    public function setProductPrices (&$row)
    {
        ...
        if ($this->_taxCalculationMethod == PS_TAX_EXC)
            $row['product_price_wt'] = Tools::ps_round(
            $row['product_price'] * (1 + ($row['tax_rate'] * 0.01)), 2) +
             Tools::ps_round(
    ...
    public function getDiscounts ($details = false)
    {
		...
		FROM `' . _DB_PREFIX_ .
         'order_discount` od ' .
         ($details ? 'LEFT JOIN `' . _DB_PREFIX_ .
         'discount` d ON (d.`id_discount` = od.`id_discount`)' : '') . '
		WHERE od.`id_order` = ' . (int) ($this->id));
    }
    public static function getDiscountsCustomer ($id_customer, $id_discount)
    {
        ...
        '
			...
			LEFT JOIN ' .
         _DB_PREFIX_ . 'order_discount od ON (od.id_order = o.id_order)
    ...
    public function getCurrentStateFull ($id_lang)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'order_state_lang` osl ON (osl.`id_order_state` = oh.`id_order_state`)
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'order_state` os ON (os.`id_order_state` = oh.`id_order_state`)
		WHERE osl.`id_lang` = ' .
         (int) ($id_lang) . ' AND oh.`id_order` = ' . (int) ($this->id) . '
    ...
    $showHiddenStatus = false)
    {
        ...
        '
		SELECT o.*, (SELECT SUM(od.`product_quantity`) FROM `' .
         _DB_PREFIX_ . 'order_detail` od WHERE od.`id_order` = o.`id_order`) nb_products
		FROM `' . _DB_PREFIX_ . 'orders` o
		WHERE o.`id_customer` = ' . (int) $id_customer . '
		GROUP BY o.`id_order`
    ...
    $id_customer = NULL, $type = NULL)
    {
		...
		FROM `' . _DB_PREFIX_ .
         'orders`
		WHERE DATE_ADD(date_upd, INTERVAL -1 DAY) <= \'' .
         pSQL($date_to) . '\' AND date_upd >= \'' . pSQL($date_from) . '\'' .
	...
	*/
    ...
    public static function getOrdersWithInformations ($limit = NULL)
    {
        ...
        '
         ...
         'order_history` oh
				LEFT JOIN `' .
         _DB_PREFIX_ . 'order_state_lang` osl ON (osl.`id_order_state` = oh.`id_order_state`)
    ...
    $id_customer = NULL, $type = NULL)
    {
		...
		FROM `' . _DB_PREFIX_ .
         'orders`
		WHERE DATE_ADD(invoice_date, INTERVAL -1 DAY) <= \'' .
         pSQL($date_to) . '\' AND invoice_date >= \'' . pSQL($date_from) . '\'' .
         ($type ? ' AND ' . pSQL(strval($type)) . '_number != 0' : '') .
         ($id_customer ? ' AND id_customer = ' . (int) ($id_customer) : '') .
    ...
    public function isReturnable ()
    {
        ...
        Configuration::get('PS_OS_DELIVERED'));
        if ($payment and $delivred and
         strtotime($delivred[0]['date_add']) < strtotime(
        $payment[0]['date_add']))
    ...
    public function setInvoice ()
    {
		...
		UPDATE `' . _DB_PREFIX_ .
         'orders`
		SET `invoice_number` = ' .
         $number . ', `invoice_date` = \'' . date('Y-m-d H:i:s') . '\'
		WHERE `id_order` = ' . (int) $this->id);
    ...
    public static function printPDFIcons ($id_order, $tr)
    {
        ...
        if ($orderState->delivery and $order->delivery_number)
            echo '<a href="pdf.php?id_delivery=' . (int) ($order->delivery_number) .
