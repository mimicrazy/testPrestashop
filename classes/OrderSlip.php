...
class OrderSlipCore extends ObjectModel
{
    ...
    public function addSlipDetail ($orderDetailList, $productQtyList)
    {
        foreach ($orderDetailList as $key => $orderDetail) {
            if ($qty = (int) ($productQtyList[$key]))
                Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'order_slip_detail', 
    ...
    $id_order_detail = false)
    {
        ...
        ($id_order_detail ? 'SELECT SUM(`product_quantity`) AS `total`' : 'SELECT *') .
         ...
         ($id_order_detail ? ' WHERE `id_order_detail` = ' .
         (int) ($id_order_detail) : ''));
    }
                ...
                if (sizeof($discounts)) {
                    ...
                    foreach ($discounts as $discount) {
                        ...
                        elseif ($discount['id_discount_type'] == 2)
                            $resTab[$key]['product_price'] -= (($discount['value'] *
    ...
    public function getProducts ()
    {
		...
		FROM `' . _DB_PREFIX_ .
         'order_slip_detail` osd
		INNER JOIN `' .
         _DB_PREFIX_ . 'order_detail` od ON osd.id_order_detail = od.id_order_detail
    ...
    public static function getSlipsIdByDate ($dateFrom, $dateTo)
    {
		...
		WHERE `date_add` BETWEEN \'' .
         pSQL($dateFrom) . ' 00:00:00\' AND \'' . pSQL($dateTo) . ' 23:59:59\'
