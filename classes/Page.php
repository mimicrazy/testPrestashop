...
class PageCore extends ObjectModel
{
        ...
        if (array_key_exists($phpSelf, $specialArray)) {
            ...
            if ($result['id_page'])
				...
				INSERT INTO `' . _DB_PREFIX_ .
                 'page` (`id_page_type`,`id_object`)
				VALUES ((SELECT pt.`id_page_type` FROM `' .
                 _DB_PREFIX_ . 'page_type` pt WHERE pt.`name` = \'' .
                 pSQL($phpSelf) . '\'),
						' . (int) ($id_object) . ')');
    ...
    public static function setPageViewed ($id_page)
    {
        ...
        if (Db::getInstance()->Affected_Rows() == 0)
            ...
            '
			INSERT INTO `' .
             _DB_PREFIX_ . 'page_viewed` (`id_date_range`,`id_page`,`counter`)
			VALUES (' .
             (int) ($id_date_range) . ',' . (int) ($id_page) . ',1)');
    }
}
