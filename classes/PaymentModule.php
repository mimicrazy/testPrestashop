...
abstract class PaymentModuleCore extends Module
{
    ...
    public function install ()
    {
        ...
        if ($this->currencies_mode == 'checkbox') {
			...
			INSERT INTO `' .
             _DB_PREFIX_ .
             'module_currency` (id_module, id_currency)
			SELECT ' .
             (int) ($this->id) . ', id_currency FROM `' . _DB_PREFIX_ .
            ...
            '
			INSERT INTO `' .
             _DB_PREFIX_ . 'module_currency` (id_module, id_currency)
		...
		INSERT INTO `' .
         _DB_PREFIX_ .
         'module_country` (id_module, id_country)
		SELECT ' .
         (int) ($this->id) . ', id_country FROM `' . _DB_PREFIX_ .
		...
		SELECT ' .
         (int) ($this->id) . ', id_group FROM `' . _DB_PREFIX_ . 'group`');
        return $return;
    }
            ...
            if ($result and isset($order->id)) {
                ...
                foreach ($products as $key => $product) {
                    ...
                    $specificPrice['reduction'], $order->id_currency) : $specificPrice['reduction']) : 0.00) . ',
                     ...
                     ',
						' .
                     (empty($product['ean13']) ? 'NULL' : '\'' .
                     pSQL($product['ean13']) . '\'') . ',
						' .
                     (empty($product['upc']) ? 'NULL' : '\'' .
                     pSQL($product['upc']) . '\'') .
                     ',
						' .
                ...
                foreach ($discounts as $discount) {
                    ...
                    if ($objDiscount->id_discount_type == 2 and
                     in_array($objDiscount->behavior_not_exhausted, array(1, 2)))
                        $shrunk = true;
                    if ($shrunk and
                     ($total_discount_value + $value) >
                     ($order->total_products_wt + $order->total_shipping +
