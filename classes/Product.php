...
class ProductCore extends ObjectModel
{
    ...
    public function updatePosition ($way, $position)
    {
        ...
        '
         ...
         (int) (Tools::getValue('id_category', 1)) . '
        ...
        '
         ...
         ($way ? '- 1' : '+ 1') . '
			WHERE `position`
			' .
         ($way ? '> ' . (int) ($movedProduct['position']) . ' AND `position` <= ' .
         (int) ($position) : '< ' . (int) ($movedProduct['position']) .
         ' AND `position` >= ' . (int) ($position)) . '
			AND `id_category`=' .
         (int) ($movedProduct['id_category'])) and Db::getInstance()->Execute(
        '
			UPDATE `' . _DB_PREFIX_ . 'category_product`
			SET `position` = ' . (int) ($position) . '
			WHERE `id_product` = ' .
         (int) ($movedProduct['id_product']) . '
			AND `id_category`=' .
         (int) ($movedProduct['id_category'])));
    }
    /*
    ...
    $minimumQuantity = 0)
    {
		...
		FROM `' . _DB_PREFIX_ .
         'product_attribute`
		WHERE `default_on` = 1 ' .
         ((int) ($minimumQuantity) > 0 ? 'AND `quantity` >= ' .
        ...
        if (! $result)
			...
			FROM `' . _DB_PREFIX_ .
             'product_attribute`
			WHERE ' .
             ((int) ($minimumQuantity) > 0 ? '`quantity` >= ' .
             (int) ($minimumQuantity) . ' AND ' : '') . '`id_product` = ' .
             (int) ($id_product));
        if (! $result)
            $result = Db::getInstance()->getRow(
			...
			WHERE `default_on` = 1 AND `id_product` = ' .
             (int) ($id_product));
        if (! $result)
            $result = Db::getInstance()->getRow(
            '
    ...
    $orderWay, $id_category = false, $only_active = false)
    {
		...
		WHERE pl.`id_lang` = ' . (int) ($id_lang) .
         ...
         '
		ORDER BY ' .
         (isset($orderByPrefix) ? pSQL($orderByPrefix) . '.' : '') . '`' .
    ...
    public function getDefaultCategoryProducts ($idLang = NULL, $limit = NULL)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ . 'product_lang` pl ON (pl.`id_product` = p.`id_product`)
		LEFT JOIN `' .
    ...
    $supplier_reference, $ean13, $default, $location = NULL, $upc = NULL)
    {
        ...
        $supplier_reference, $ean13, $default, $location, $upc) or
         ! Db::getInstance()->Execute(
    ...
    public function setDefaultAttribute ($id_product_attribute)
    {
		...
		AND `id_product_attribute` = ' .
         (int) $id_product_attribute) && Db::getInstance()->Execute(
        '
    ...
    $minimal_quantity)
    {
        ...
        $data, 'UPDATE', 
        '`id_product_attribute` = ' . (int) ($id_product_attribute)) or
         ! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
    ...
    public function getCombinationImages ($id_lang)
    {
        ...
        '
			...
			LEFT JOIN `' .
         _DB_PREFIX_ . 'image_lang` il ON (il.`id_image` = pai.`id_image`)
			LEFT JOIN `' .
         _DB_PREFIX_ .
         'image` i ON (i.`id_image` = pai.`id_image`)
			WHERE pai.`id_product_attribute` IN (' .
         implode(', ', $ids) . ') AND il.`id_lang` = ' . (int) ($id_lang) .
         ' ORDER by i.`position`'))
            return false;
        $images = array();
        foreach ($result as $row)
    ...
    $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL)
    {
		...
		SELECT p.*, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, p.`ean13`, p.`upc`,
			i.`id_image`, il.`legend`, t.`rate`, m.`name` AS manufacturer_name, DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL ' .
         (Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get(
		...
		ORDER BY ' .
         ...
         pSQL($orderBy) . '` ' . pSQL($orderWay) . '
		LIMIT ' .
         (int) ($pageNumber * $nbProducts) . ', ' . (int) ($nbProducts));
        if ($orderBy == 'price')
    ...
    $ending = false)
    {
        ...
        if ($count) {
			...
			AND p.`show_price` = 1
			' .
             ((! $beginning and ! $ending) ? ' AND p.`id_product` IN(' .
             ((is_array($ids_product) and sizeof($ids_product)) ? implode(', ', 
    ...
    public static function getProductCategoriesFull ($id_product = '', $id_lang)
    {
        ...
        '
		SELECT cp.`id_category`, cl.`name`, cl.`link_rewrite` FROM `' .
         _DB_PREFIX_ .
         'category_product` cp
		LEFT JOIN `' .
         _DB_PREFIX_ . 'category_lang` cl ON (cp.`id_category` = cl.`id_category`)
    ...
    $with_ecotax, &$specific_price, $use_groupReduction)
    {
        ...
        $cacheId = $id_product . '-' . $id_shop . '-' . $id_currency . '-' .
         ...
         $quantity . '-' . $product_attribute_label . '-' . ($use_tax ? '1' : '0') .
         '-' . $decimals . '-' . ($only_reduc ? '1' : '0') . '-' .
    ...
    public static function isDiscounted ($id_product, $quantity = 1)
    {
        ...
        '
         ...
         'cart_product`
			WHERE `id_product` = ' .
         (int) ($id_product) . ' AND `id_cart` = ' . (int) ($cart->id));
    ...
    static function productPrice ($params, &$smarty)
    {
        ...
        if (isset($params['p']['reduction']) and $params['p']['reduction'])
            $ret .= '<span class="discounted">' .
             Tools::displayPrice($params['p']['price_without_reduction'], 
            $smarty->ps_currency) . '</span><br />';
    ...
    $id_product_attribute = NULL, $cache_is_pack = NULL)
    {
        ...
        if (((int) $cache_is_pack or
         ($cache_is_pack === NULL and Pack::isPack((int) $id_product, 
        (int) $lang))) and ! Pack::isInStock((int) $id_product, (int) $lang))
            return 0;
		...
		FROM `' . _DB_PREFIX_ .
         'product` p
		LEFT JOIN `' .
         _DB_PREFIX_ . 'product_attribute` AS pa ON pa.`id_product` = p.`id_product`
		WHERE p.`id_product` = ' . (int) ($id_product) .
         '
		' .
    ...
    public function getAttributesGroups ($id_lang)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'attribute_lang` al ON a.`id_attribute` = al.`id_attribute`
		LEFT JOIN `' .
    ...
    public static function getAccessoriesLight ($id_lang, $id_product)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'product` p ON (p.`id_product`= `id_product_2`)
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'product_lang` pl ON (p.`id_product` = pl.`id_product` AND pl.`id_lang` = ' .
         (int) ($id_lang) . ')
    ...
    public function addFeaturesCustomToDB ($id_value, $lang, $cust)
    {
        ...
        return Db::getInstance()->autoExecute(_DB_PREFIX_ . 'feature_value_lang', 
        $row, 'INSERT');
    }
    ...
    $id_feature_value)
    {
        ...
        '
			INSERT INTO `' .
         _DB_PREFIX_ .
         'feature_product` (`id_feature`, `id_product`, `id_feature_value`)
			VALUES (' .
         (int) ($id_feature) . ', ' . (int) ($id_product) . ', ' .
         (int) ($id_feature_value) . ')
			ON DUPLICATE KEY UPDATE `id_feature_value` = ' .
         (int) ($id_feature_value));
    }
    /**
    ...
    public static function cacheFrontFeatures ($productIds, $id_lang)
    {
        ...
        foreach ($productIds as $id_product)
            if ((int) $id_product and
             ! array_key_exists($id_product . '-' . $id_lang, 
            self::$_cacheFeatures))
                $productImplode[] = (int) $id_product;
        if (! count($productImplode))
            return;
    ...
    public static function searchByName ($id_lang, $query)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'product_attribute` pa ON pa.`id_product` = p.`id_product`
		WHERE pl.`name` LIKE \'%' .
         pSQL($query) . '%\' OR p.`reference` LIKE \'%' . pSQL($query) .
    ...
    public static function duplicateDownload ($id_product_old, $id_product_new)
    {
        ...
        'SELECT `display_filename`, `physically_filename`, `date_deposit`, `date_expiration`,
			...
			FROM `' .
         _DB_PREFIX_ . 'product_download` WHERE `id_product` = ' .
    ...
    public static function getProductProperties ($id_lang, $row)
    {
        ...
        if ((! isset($row['id_product_attribute']) or
         ! $row['id_product_attribute']) and
         ((isset($row['cache_default_attribute']) and
         ($ipa_default = $row['cache_default_attribute']) !== NULL) or
         ($ipa_default = Product::getDefaultAttribute($row['id_product'], 
        ! $row['allow_oosp']))))
        ...
        $row['id_product_attribute'])) : 0;
        $row['price_tax_exc'] = Product::getPriceStatic((int) $row['id_product'], 
        false, 
	...
	*/
    ...
    public static function getAttachmentsStatic ($id_lang, $id_product)
    {
		...
		LEFT JOIN ' .
         _DB_PREFIX_ .
         'attachment a ON a.id_attachment = pa.id_attachment
		LEFT JOIN ' .
         _DB_PREFIX_ .
	...
	*/
    ...
    public function deleteCustomizedDatas ($id_customization)
    {
        ...
        return (Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization` WHERE `id_customization` = ' . (int) ($id_customization)) and
         Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customized_data` WHERE `id_customization` = ' .
    ...
    public static function addCustomizationPrice (&$products, &$customizedDatas)
    {
        foreach ($products as &$productUpdate) {
            ...
            $priceWt = $price *
             (1 +
             ((isset($productUpdate['tax_rate']) ? $productUpdate['tax_rate'] : $productUpdate['rate']) *
             0.01));
	...
	*/
    ...
    protected function _deleteOldLabels ()
    {
        ...
        if ($extraFile > 0 and
         count($customizationFields[_CUSTOMIZE_FILE_]) - $extraFile >= 0 and
         (! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field` WHERE `id_product` = ' . (int) ($this->id) .
         ' AND `type` = ' . _CUSTOMIZE_FILE_ .
         ' AND `id_customization_field` >= ' .
         (int) ($customizationFields[_CUSTOMIZE_FILE_][count(
        $customizationFields[_CUSTOMIZE_FILE_]) - $extraFile])) or
         ! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field_lang` WHERE `id_customization_field` NOT IN (SELECT `id_customization_field` FROM `' .
         _DB_PREFIX_ . 'customization_field`)')))
            return false;
        if ($extraText > 0 and
         count($customizationFields[_CUSTOMIZE_TEXTFIELD_]) - $extraText >= 0 and
         (! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field` WHERE `id_product` = ' . (int) ($this->id) .
         ' AND `type` = ' . _CUSTOMIZE_TEXTFIELD_ .
         ' AND `id_customization_field` >= ' .
         (int) ($customizationFields[_CUSTOMIZE_TEXTFIELD_][count(
        $customizationFields[_CUSTOMIZE_TEXTFIELD_]) - $extraText])) or
         ! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field_lang` WHERE `id_customization_field` NOT IN (SELECT `id_customization_field` FROM `' .
         _DB_PREFIX_ . 'customization_field`)')))
    ...
    {
        global $cookie;
        $fields = array_merge($cookie->getFamily('pictures_' . (int) ($this->id)), 
        $cookie->getFamily('textFields_' . (int) ($this->id)));
        if (($requiredFields = $this->getRequiredCustomizableFields()) === false)
            return false;
        $prefix = array(_CUSTOMIZE_FILE_ => 'pictures_' . (int) ($this->id) .
         '_', _CUSTOMIZE_TEXTFIELD_ => 'textFields_' . (int) ($this->id) . '_');
    ...
    public function checkAccess ($id_customer)
    {
        if (! $id_customer)
			...
			FROM `' . _DB_PREFIX_ .
             'category_product` cp
			INNER JOIN `' .
             _DB_PREFIX_ . 'category_group` ctg ON (ctg.`id_category` = cp.`id_category`)
			WHERE cp.`id_product` = ' .
             (int) $this->id . ' AND ctg.`id_group` = 1');
        else
            return (bool) Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            '
			SELECT cg.`id_group`
			...
			INNER JOIN `' .
             _DB_PREFIX_ .
             'category_group` ctg ON (ctg.`id_category` = cp.`id_category`)
			INNER JOIN `' .
             _DB_PREFIX_ .
             'customer_group` cg ON (cg.`id_group` = ctg.`id_group`)
			WHERE cp.`id_product` = ' .
             (int) $this->id . ' AND cg.`id_customer` = ' . (int) $id_customer);
    }
    /**
    ...
    public function getStockMvts ($id_lang)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'employee` e ON (e.id_employee = sm.id_employee)
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'product_attribute_combination` pac ON (pac.id_product_attribute = sm.id_product_attribute)
		LEFT JOIN `' .
         _DB_PREFIX_ .
    ...
    public static function getUrlRewriteInformations ($id_product)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'lang` l ON (pl.`id_lang` = l.`id_lang`)
		LEFT JOIN `' .
         _DB_PREFIX_ . 'category_lang` cl ON (cl.`id_category` = p.`id_category_default`  AND cl.`id_lang` = pl.`id_lang`)
		WHERE p.`id_product` = ' . (int) $id_product . '
		AND l.`active` = 1');
    }
    ...
    public function setCoverWs ($id_image)
    {
        ...
        'UPDATE `' . _DB_PREFIX_ . 'image`
									SET `cover` = 0 WHERE `id_product` = ' .
         (int) ($this->id) . '
									');
        Db::getInstance()->Execute(
        'UPDATE `' . _DB_PREFIX_ .
         'image`
									SET `cover` = 1 WHERE `id_product` = ' .
         (int) ($this->id) . ' AND `id_image` = ' . (int) $id_image);
        return true;
    }
    /**
