...
class ReferrerCore extends ObjectModel
{
    ...
    public static function cacheNewSource ($id_connections_source)
    {
        ...
        '
		INSERT INTO ' .
         _DB_PREFIX_ . 'referrer_cache (id_referrer, id_connections_source) (
			SELECT id_referrer, id_connections_source
			FROM ' . _DB_PREFIX_ . 'referrer r
			LEFT JOIN ' .
         _DB_PREFIX_ . 'connections_source cs ON (' . self::$_join . ')
			WHERE id_connections_source = ' .
         (int) ($id_connections_source) . '
		)');
    }
    public static function getReferrers ($id_customer)
    {
		...
		LEFT JOIN ' .
         _DB_PREFIX_ .
         'connections c ON c.id_guest = g.id_guest
		LEFT JOIN ' .
         _DB_PREFIX_ . 'connections_source cs ON c.id_connections = cs.id_connections
		LEFT JOIN ' . _DB_PREFIX_ .
         'referrer r ON (' . self::$_join . ')
    ...
    public function getStatsVisits ($id_product = null, $employee = null)
    {
        ...
        if ((int) ($id_product)) {
            $join = 'LEFT JOIN `' . _DB_PREFIX_ .
             'page` p ON cp.`id_page` = p.`id_page`
					 LEFT JOIN `' .
             _DB_PREFIX_ .
		...
		LEFT JOIN ' .
         _DB_PREFIX_ .
         'connections_source cs ON rc.id_connections_source = cs.id_connections_source
		LEFT JOIN ' .
         _DB_PREFIX_ .
         'connections c ON cs.id_connections = c.id_connections
		LEFT JOIN ' .
         _DB_PREFIX_ . 'connections_page cp ON cp.id_connections = c.id_connections
		' . $join . '
		WHERE cs.date_add BETWEEN ' .
    ...
    public function getRegistrations ($id_product = null, $employee = null)
    {
		...
		LEFT JOIN ' .
         ...
         'connections_source cs ON rc.id_connections_source = cs.id_connections_source
		LEFT JOIN ' .
         _DB_PREFIX_ . 'connections c ON cs.id_connections = c.id_connections
		LEFT JOIN ' . _DB_PREFIX_ . 'guest g ON g.id_guest = c.id_guest
		LEFT JOIN ' .
         _DB_PREFIX_ . 'customer cu ON cu.id_customer = g.id_customer
		' . $join . '
		WHERE cu.date_add BETWEEN ' .
         ModuleGraph::getDateBetween($employee) . '
    ...
    public function getStatsSales ($id_product = null, $employee = null)
    {
		...
		LEFT JOIN ' .
         _DB_PREFIX_ . 'orders oo ON oo.id_customer = g.id_customer
		' . $join . '
		WHERE oo.invoice_date BETWEEN ' .
         ModuleGraph::getDateBetween($employee) . '
    ...
    $employee = null)
    {
        ...
        $jsonArray[] = '"base_fee":"' .
         Tools::displayPrice($statsSales['orders'] * $referrer->base_fee, 
        $currency) . '"';
        $jsonArray[] = '"percent_fee":"' .
         Tools::displayPrice(
