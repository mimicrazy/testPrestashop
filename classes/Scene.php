...
class SceneCore extends ObjectModel
{
    ...
    public function addZoneProducts ($zones)
    {
        ...
        foreach ($zones as $zone) {
            $sql = 'INSERT INTO `' . _DB_PREFIX_ .
             'scene_products` ( `id_scene` , `id_product` , `x_axis` , `y_axis` , `zone_width` , `zone_height`) VALUES
				 (' .
    ...
    $onlyActive = true, $liteResult = true, $hideScenePosition = true)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'scene_lang` sl ON (sl.id_scene = s.id_scene)
		WHERE sc.id_category = ' .
         (int) ($id_category) . '	AND sl.id_lang = ' . (int) ($id_lang) .
    ...
    public function getProducts ($onlyActive = true, $id_lang = NULL, $liteResult = true)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'product` p ON (p.id_product = s.id_product)
		WHERE s.id_scene = ' .
         (int) ($this->id) . ($onlyActive ? ' AND p.active = 1' : ''));
