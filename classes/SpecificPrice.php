...
class SpecificPriceCore extends ObjectModel
{
    ...
    public static function getByProductId ($id_product)
    {
        ...
        '
			SELECT * FROM `' .
         _DB_PREFIX_ . 'specific_price` WHERE `id_product` = ' . (int) $id_product);
    }
    public static function getIdsByProductId ($id_product)
    {
        return Db::getInstance()->ExecuteS(
        '
			SELECT `id_specific_price` FROM `' .
         _DB_PREFIX_ . 'specific_price` WHERE `id_product` = ' . (int) $id_product . '
		');
    }
    ...
    public static function setSpecificPriority ($id_product, $priorities)
    {
        ...
        '
		INSERT INTO `' .
         _DB_PREFIX_ . 'specific_price_priority` (`id_product`, `priority`)
		VALUES (' .
         (int) $id_product . ',\'' . pSQL(rtrim($value, ';')) . '\')
    ...
    $id_currency, $id_country, $id_group)
    {
        ...
        '
			SELECT *,
					' .
         self::_getScoreQuery($id_product, $id_shop, $id_currency, $id_country, 
        $id_group) . '
			FROM `' . _DB_PREFIX_ . 'specific_price`
    ...
    $id_country, $id_group, $beginning, $ending)
    {
        ...
        '
			...
			WHERE	`id_shop` IN(0, ' . (int) $id_shop . ') AND
					...
					`from_quantity` = 1 AND
					(
						(`from` = \'0000-00-00 00:00:00\' OR \'' .
         pSQL($beginning) . '\' >= `from`)
						AND
						(`to` = \'0000-00-00 00:00:00\' OR \'' .
         pSQL($ending) . '\' <= `to`)
					)
					AND
					`reduction` > 0
