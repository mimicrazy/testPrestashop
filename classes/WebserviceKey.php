...
class WebserviceKeyCore extends ObjectModel
{
    ...
    public function deleteAssociations ()
    {
        ...
        '
				...
				WHERE `id_webservice_account` = ' .
         (int) ($this->id)) === false || Db::getInstance()->Execute(
        '
				DELETE FROM `' . _DB_PREFIX_ . 'webservice_permission`
				WHERE `id_webservice_account` = ' .
         (int) ($this->id)) === false)
    ...
    public static function getPermissionForAccount ($auth_key)
    {
        ...
        '
			SELECT p.*
			FROM `' . _DB_PREFIX_ .
         'webservice_permission` p
			LEFT JOIN `' .
         _DB_PREFIX_ . 'webservice_account` a ON (a.id_webservice_account = p.id_webservice_account)
