...
class WebserviceOutputBuilderCore
{
    ...
    $fields_to_display = 'minimum', $depth = 0, $type_of_view = self::VIEW_LIST, $override = true)
    {
        ...
        if ($type_of_view === self::VIEW_LIST)
            $output .= $this->setIndent($depth) .
             $this->objectRender->renderNodeHeader(
        ...
        if ($type_of_view === self::VIEW_LIST)
            $output .= $this->setIndent($depth) .
             $this->objectRender->renderNodeFooter(
            $ws_params['objectsNodeName'], $ws_params);
        if ($override)
            $output = $this->objectRender->overrideContent($output);
        return $output;
    ...
    public function renderEntityMinimum ($object, $depth)
    {
        ...
        $output = $this->setIndent($depth) .
         $this->objectRender->renderNodeHeader($ws_params['objectNodeName'], 
        $ws_params, $more_attr, false);
        return $output;
    }
    /**
    ...
    public function renderEntity ($object, $depth)
    {
        ...
        $output .= $this->setIndent($depth) .
         $this->objectRender->renderNodeFooter($ws_params['objectNodeName'], 
        $ws_params);
        return $output;
    }
    /**
        ...
        foreach ($associations as $assoc_name => $association) {
            if ($this->fieldsToDisplay == 'full' ||
             ...
             array_key_exists($assoc_name, $this->fieldsToDisplay)) {
                ...
                if ($output_details != '') {
                    $output .= $this->setIndent($depth) .
                     $this->objectRender->renderAssociationHeader($object, 
                    $ws_params, $assoc_name);
                    $output .= $output_details;
                    $output .= $this->setIndent($depth) .
                     $this->objectRender->renderAssociationFooter($object, 
                    $ws_params, $assoc_name);
                } else {
                    $output .= $this->setIndent($depth) .
                     $this->objectRender->renderAssociationHeader($object, 
                    $ws_params, $assoc_name, true);
                }
            }
        }
