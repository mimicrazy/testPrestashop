...
class WebserviceRequestCore
{
    ...
    $code)
    {
        $this->setError($num, 
        $label . '. Did you mean: "' .
         $this->getClosest($value, $available_values) . '"?' .
         (count($available_values) > 1 ? ' The full list is: "' .
    ...
    protected function checkHTTPMethod ()
    {
        ...
        elseif (isset($this->urlSegment[0]) &&
         isset($this->resourceList[$this->urlSegment[0]]['forbidden_method']) &&
             ...
             $this->fieldsToDisplay != 'minimum') {
                ...
                if (count($matches)) {
                    ...
                    foreach ($fieldsToTest as $field_name => $part) {
                        ...
                        if ($error) {
                            $this->setError(400, 
                            'Unable to display this field "' . $field_name .
                             (is_array($part) ? ' (details : ' .
                    ...
                    $this->setError(400, 
                    'The \'display\' syntax is wrong. You can set \'full\' or \'[field_1,field_2,field_3,...]\'. These are available: ' .
                     implode(', ', 
    ...
    protected function manageFilters ()
    {
        ...
        foreach ($this->resourceConfiguration['fields'] as $fieldName => $field)
            if ((! isset($this->resourceConfiguration['hidden_fields']) ||
             (isset($this->resourceConfiguration['hidden_fields']) &&
             ! in_array($fieldName, 
            $this->resourceConfiguration['hidden_fields']))))
                if ((! isset($field['i18n']) ||
    ...
    protected function saveEntityFromXml ($successReturnCode)
    {
        ...
        foreach ($xmlEntities as $xmlEntity) {
            ...
            foreach ($this->resourceConfiguration['fields'] as $fieldName => $fieldProperties) {
                ...
                if (isset($attributes->$fieldName) &&
                 ...
                 $fieldProperties['required'] && ! $fieldProperties['i18n']) {
                    $this->setError(400, 'parameter "' . $fieldName .
                     '" required', 41);
                    return false;
                } elseif ((! isset($fieldProperties['required']) ||
            ...
            if (! $this->hasErrors()) {
                if ($i18n &&
                 ($retValidateFieldsLang = $object->validateFieldsLang(false, 
                true)) !== true) {
                    $this->setError(400, 
                ...
                true)) !== true) {
                    ...
                    if (isset($this->resourceConfiguration['objectMethods']) &&
                     array_key_exists($objectMethod, 
                    $this->resourceConfiguration['objectMethods']))
