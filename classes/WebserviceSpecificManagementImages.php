	...
	*/
    ...
    private function manageGeneralImages ()
    {
        ...
        switch ($this->wsObject->urlSegment[2]) {
            ...
            case 'header':
                if (in_array($this->wsObject->method, array('GET', 'HEAD', 
                'PUT')))
    ...
    $normal_image_sizes)
    {
        ...
        if ($this->imageType == 'products') {
            ...
            if ($this->wsObject->urlSegment[3] != '') {
                ...
                if (! Validate::isUnsignedId($object_id) ||
                 ! in_array($this->wsObject->urlSegment[3], 
                $available_image_ids))
                    ...
                    $image_size = $this->wsObject->urlSegment[4];
                    if (file_exists(
                    $directory . $path . '/' . $image_id .
                     (strlen($this->wsObject->urlSegment[4]) > 0 ? '-' .
                     $this->wsObject->urlSegment[4] : '') . '.jpg')) {
                        $filename = $directory . $path . '/' . $image_id .
                         (strlen($this->wsObject->urlSegment[4]) > 0 ? '-' .
                         $this->wsObject->urlSegment[4] : '') . '.jpg';
                    ...
                    } else // else old system or not exists
{
    ...
    $destHeight = NULL, $imageTypes = NULL, $parentPath = NULL)
    {
        ...
        if ($widthDiff > 1 and $heightDiff > 1) {
            ...
            if ((int) (Configuration::get('PS_IMAGE_GENERATION_METHOD')) == 2 or
             ((int) (Configuration::get('PS_IMAGE_GENERATION_METHOD')) == 0 and
        ...
        255)) || ! imagefill($destImage, 0, 0, $white) ||
         ! imagecopyresampled($destImage, $sourceImage, $borderWidth, 
        $borderHeight, 0, 0, $nextWidth, $nextHeight, $sourceWidth, 
        $sourceHeight) || ! imagecolortransparent($destImage, $white))
            throw new WebserviceException(
            ...
            if (isset($_FILES['image']['tmp_name']) and
             $_FILES['image']['tmp_name']) {
                ...
                if ($error = checkImageUploadError($file))
                    throw new WebserviceException('Image upload error : ' .
                     $error, array(76, 400));
                if (isset($file['tmp_name']) and $file['tmp_name'] != NULL) {
                    ...
                    if ($this->imageType == 'products') {
                        ...
                        if (! (Configuration::get('PS_OLD_FILESYSTEM') &&
                         file_exists(
