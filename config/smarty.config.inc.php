...
smartyRegisterFunction($smarty, 'modifier', 'truncate', 
'smarty_modifier_truncate');
smartyRegisterFunction($smarty, 'modifier', 'secureReferrer', 
array('Tools', 'secureReferrer'));
...
array('Tools', 'dateFormat'));
smartyRegisterFunction($smarty, 'function', 'productPrice', 
array('Product', 'productPrice')); // unused
smartyRegisterFunction($smarty, 'function', 'convertPrice', 
array('Product', 'convertPrice'));
smartyRegisterFunction($smarty, 'function', 'convertPriceWithoutDisplay', 
array('Product', 'productPriceWithoutDisplay')); // unused
smartyRegisterFunction($smarty, 'function', 'convertPriceWithCurrency', 
array('Product', 'convertPriceWithCurrency'));
smartyRegisterFunction($smarty, 'function', 'displayWtPrice', 
array('Product', 'displayWtPrice'));
smartyRegisterFunction($smarty, 'function', 'displayWtPriceWithCurrency', 
array('Product', 'displayWtPriceWithCurrency'));
smartyRegisterFunction($smarty, 'function', 'displayPrice', 
array('Tools', 'displayPriceSmarty'));
smartyRegisterFunction($smarty, 'modifier', 'convertAndFormatPrice', 
array('Product', 'convertAndFormatPrice')); // used twice
function smartyTranslate ($params, &$smarty)
{
    ...
    if (Configuration::get('PS_FORCE_SMARTY_2')) /* Keep a backward compatibility for Smarty v2 */
        ...
        $filename = ((! isset($smarty->compiler_object) or
         ! is_object($smarty->compiler_object->template)) ? $smarty->template_filepath : $smarty->compiler_object->template->getTemplateFilepath());
    ...
    if ($params['mod']) {
        ...
        _PS_THEME_DIR_ . 'modules/' . $params['mod'] . '/' . $iso . '.php')) {
            $translationsFile = _PS_THEME_DIR_ . 'modules/' . $params['mod'] .
             '/' . $iso . '.php';
            $key = '<{' . $params['mod'] . '}' . _THEME_NAME_ . '>' . $key;
        } else {
            $translationsFile = _PS_MODULE_DIR_ . $params['mod'] . '/' . $iso .
             '.php';
    ...
    if (is_array($lang_array) and key_exists($key, $lang_array))
        $msg = $lang_array[$key];
    elseif (is_array($lang_array) and
     key_exists(Tools::strtolower($key), $lang_array))
...
function smartyMaxWords ($params, &$smarty)
{
    ...
    $params['s'] = str_replace('...', ' ...', 
    html_entity_decode($params['s'], ENT_QUOTES, 'UTF-8'));
    $words = explode(' ', $params['s']);
    foreach ($words as &$word)
        if (Tools::strlen($word) > $params['n'])
            $word = Tools::substr(
            trim(chunk_split($word, $params['n'] - 1, '- ')), 0, - 1);
...
function smartyTruncate ($params, &$smarty)
{
    ...
    return (isset($params['encode']) ? Tools::htmlentitiesUTF8($text, 
    ENT_NOQUOTES) : $text);
}
...
$break_words = false, $middle = false, $charset = 'UTF-8')
{
    ...
    if (Tools::strlen($string) > $length) {
        ...
        if (! $break_words && ! $middle)
            ...
            Tools::substr($string, 0, $length + 1, $charset));
        return ! $middle ? Tools::substr($string, 0, $length, $charset) . $etc : Tools::substr(
        $string, 0, $length / 2, $charset) . $etc .
         Tools::substr($string, - $length / 2, $charset);
    } else
        return $string;
}
