...
if (isset($_POST['x_exp_date']) && isset($_POST['x_exp_date_m']) &&
 isset($_POST['x_exp_date_y']) && isset($_POST['x_exp_date_y']) &&
 isset($_POST['name'])) {
...
$postString .= 'x_exp_date=' .
 str_pad($_POST["x_exp_date_m"], 2, "0", STR_PAD_LEFT) . $_POST["x_exp_date_y"];
...
if (! isset($response[7]) or ! isset($response[3]) or ! isset($response[9])) {
    Logger::addLog(
    'Authorize.net returned a malformed response for cart ' . $response[7], 4);
...
if ($response[0] == 3)
    ...
    if ($response[0] == 1)
        ...
        $authorizeaim->displayName, $message, NULL, NULL, false, 
        $customer->secure_key);
    else
        $authorizeaim->validateOrder((int) $cart->id, 
        ...
        $customer->secure_key);
    Tools::redirect(
    'order-confirmation.php?id_module=' . (int) $authorizeaim->id . '&id_cart=' .
